...
function initResetAndHelp() {
    ...
    const HELP_HTML = `
        ...
        <h3>【角色管理】</h3>
        <ul>
            <li>点击悬浮按钮组的<b>"角色管理"</b>按钮打开角色列表</li>
            <li><b>新建角色</b>：创建一张空白角色卡（当前角色会先自动保存）</li>
            <li><b>切换</b>：在角色列表点击"切换"按钮，会保存当前角色并加载目标角色</li>
            <li><b>删除</b>：移除指定角色卡（删除当前角色会自动切到下一个角色）</li>
            <li>各角色数据完全隔离，互不影响</li>
        </ul>

        <h3>【数据管理】</h3>
        <ul>
            <li><b>保存</b>：将当前角色数据保存到本地浏览器缓存</li>
            <li><b>导出</b>：将当前角色数据导出为JSON文件（选择保存路径）</li>
            <li><b>导入</b>：从JSON文件加载角色数据，可选"创建为新角色"或"覆盖当前角色"</li>
            <li><b>重置</b>：清空当前角色数据并切换到下一个角色（如存在）</li>
        </ul>
        ...
        <ul>
            ...
            <li>导入时默认创建为新角色，选择"覆盖当前角色"会替换当前角色全部内容</li>
            <li>切换/新建/删除角色时，当前角色会先自动保存</li>
        </ul>
